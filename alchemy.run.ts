 ...
 *
 * Unset includes empty, and in CI that matters: `${{ secrets.X }}` for a secret
 * nobody created is an empty string rather than an absent variable, and
 * Effect's environment provider reads an empty variable as missing.
 */
const optional = (name: string) =>
  Config.option(Config.Redacted(name)).pipe(Config.map(Option.getOrUndefined));

/** The same, for a setting that is not a secret. */
...
/**
 * What a stage is made of, apart from which Railway it is made in.
 *
 * Exported on its own so `tooling/test/deploy-offline.test.ts` can run exactly
 * this program against stand-in providers — the dry run that needs no token.
 */
export const deployment = Effect.gen(function*() {
  ...
  const workerService = yield* Service("worker", {
    ...
    env: {
      ...
      ...database,
      ...telemetry,
      WEBHOOK_CONCURRENCY: yield* setting("WEBHOOK_CONCURRENCY"),
    },
  });
