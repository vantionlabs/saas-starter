...
a sprint that skips this builds the wrong thing exactly on schedule.
...
a sprint decision made from a half-remembered statistic is a decision made from
nothing.
...
Write the feature list, then cut it to what twelve build days can hold, given
that auth, organizations, roles, tenant isolation, the audit trail, API keys and
the public API are already done.
...
- `docs/sprint/01-discovery.md` — who, the incumbent, the assumption, the evidence
